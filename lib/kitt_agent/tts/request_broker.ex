defmodule KittAgent.TTS.RequestBroker do
  ...
  require Logger
  require KittAgent.Datasets.Content
  alias KittAgent.Repo
  alias KittAgent.Datasets.{Kitt, Content, Event}
  ...
  def exec(%Content{event: %Event{kitt_id: kitt_id}} = content),
    do: cast_stub(:exec, kitt_id, [content.id])
  
  def exec(%Content{} = content) do
    content = Repo.preload(content, :event)
    if content.event do
      cast_stub(:exec, content.event.kitt_id, [content.id])
    else
      Logger.error("Content #{content.id} has no associated event/kitt")
      {:error, :no_kitt}
    end
  end

  @impl Rpcsdk.RequestBroker
  def get_key(id) when is_binary(id), do: id
  def get_key(%Kitt{id: id}), do: id

  @impl true
  def handle_cast({:exec, _kitt_id, content_id}, s) do
    # Reload content to ensure we have latest state and associations
    case Repo.get(Content, content_id) |> Repo.preload(event: :kitt) do
      %Content{event: %Event{kitt: %Kitt{} = kitt}} = content ->
        process_tts(content, kitt)
      
      _ ->
        Logger.error("TTS: Content #{content_id} not found or missing associations")
    end
    
    {:noreply, s}
  end

  defp process_tts(content, kitt) do
    {:ok, content} = Events.content_processing(content)
    
    try do
      lang_code = lang_to_code(kitt.lang)
      speaker_audio = prepare_speaker_audio(kitt.audio_path)
      ...
      with {:ok, audio_url} <- call_gradio(content.message, lang_code, speaker_audio),
           {:ok, local_path} <- download_audio(audio_url) do
  ...
  defp prepare_speaker_audio(nil), do: nil
  defp prepare_speaker_audio(path) do
    filename = Path.basename(path)
    # Check possible locations
    # 1. uploads/ (relative to project root in dev)
    # 2. /app/uploads/ (docker container absolute)
    # 3. uploads/audio/ (RequestBroker download location)
    ...
    |> case do
      nil ->
        Logger.warning("TTS: Speaker audio file not found for path: #{path}")
        nil
      real_path ->
        try do
          case upload_file(real_path) do
            {:ok, remote_path} ->
              ...
              %{
                ...
                "mime_type" => MIME.from_path(real_path),
                "is_stream" => false,
                "meta" => %{"_type" => "gradio.FileData"}
              }
            {:error, reason} ->
              Logger.error("TTS: Failed to upload speaker audio: #{inspect(reason)}")
              nil
          end
        rescue
          e ->
  ...
  defp download_audio(url) do
    ...
    local_rel_path = "/uploads/audio/#{filename}"
    local_abs_path = "uploads/audio/#{filename}" # Assumes app root is working dir, which it is
